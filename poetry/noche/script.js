...
function updateLangUI(opt) {
    ...
    localStorage.setItem('noche_lang', lang);
}
...
const savedLang = localStorage.getItem('noche_lang');
...
// --- Particle Effects (Stardust & Moonlight) ---
...
class Stardust {
    ...
    init() {
        ...
        const colors = ['rgba(220,230,255,','rgba(150,180,255,','rgba(255,240,200,','rgba(100,120,200,'];
        this.color = colors[Math.floor(Math.random() * colors.length)];
    }
    update() {
        this.x += this.speedX + Math.sin(Date.now() * 0.001 + this.pulseOffset) * 0.1;
        this.y += this.speedY;
...
function initParticles() { particles = []; for (let i = 0; i < particleCount; i++) particles.push(new Stardust()); }
